...
def seed_database():
    ...
    try:
        ...
        if existing_recipes > 0:
            # Проверяем, есть ли уже рецепты закусок
            existing_snacks = db.query(Recipe).filter(Recipe.category == "Закуски").count()
            if existing_snacks > 0:
                print(f"В базе данных уже есть {existing_recipes} рецептов, включая {existing_snacks} закусок. Пропускаем заполнение.")
                return
            else:
                print(f"В базе данных уже есть {existing_recipes} рецептов. Добавляем только рецепты закусок...")
                # Пропускаем основные рецепты, переходим к закускам
                skip_to_snacks = True
        else:
            print("База данных пуста. Добавляем все рецепты...")
            skip_to_snacks = False
        
        if not skip_to_snacks:
        ...
        # Рецепт 7: Брускетта с томатами и базиликом (закуски - добавляются всегда, если их нет)
        recipe7 = Recipe(
            ...
            servings=4,
            calories_per_serving=180,
            proteins_per_serving=6.0,
            fats_per_serving=8.0,
            carbohydrates_per_serving=22.0,
            image="https://images.unsplash.com/photo-1572441713132-51c75654db73?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&q=80&w=1080"
        )
        db.add(recipe7)
        db.flush()
        
        ingredients7 = [
            Ingredient(recipe_id=recipe7.id, name="Белый хлеб", amount="8", unit="ломтиков", order=0),
            Ingredient(recipe_id=recipe7.id, name="Помидоры", amount="4", unit="шт", order=1),
            Ingredient(recipe_id=recipe7.id, name="Чеснок", amount="2", unit="зубчика", order=2),
            Ingredient(recipe_id=recipe7.id, name="Оливковое масло", amount="4", unit="ст.л.", order=3),
            Ingredient(recipe_id=recipe7.id, name="Свежий базилик", amount="10", unit="листьев", order=4),
            Ingredient(recipe_id=recipe7.id, name="Бальзамический уксус", amount="1", unit="ст.л.", order=5),
            Ingredient(recipe_id=recipe7.id, name="Соль и перец", amount="по", unit="вкусу", order=6),
        ]
        db.add_all(ingredients7)
        
        steps7 = [
            Step(recipe_id=recipe7.id, number=1, instruction="Нарежьте хлеб ломтиками толщиной 1 см.", image="https://images.unsplash.com/photo-1518977822534-7049a61ee0c2?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&q=80&w=1080", order=0),
            Step(recipe_id=recipe7.id, number=2, instruction="Обжарьте хлеб на сковороде или в тостере до золотистого цвета с обеих сторон.", image="https://images.unsplash.com/photo-1556910103-1c02745aae4d?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&q=80&w=1080", order=1),
            Step(recipe_id=recipe7.id, number=3, instruction="Натрите каждый ломтик разрезанным зубчиком чеснока.", image="https://images.unsplash.com/photo-1518977822534-7049a61ee0c2?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&q=80&w=1080", order=2),
            Step(recipe_id=recipe7.id, number=4, instruction="Нарежьте помидоры мелкими кубиками, добавьте нарезанный базилик, оливковое масло, бальзамический уксус, соль и перец.", image="https://images.unsplash.com/photo-1551185618-07fd482ff86e?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&q=80&w=1080", order=3),
            Step(recipe_id=recipe7.id, number=5, instruction="Выложите томатную смесь на хлеб и сразу подавайте.", image="https://images.unsplash.com/photo-1572441713132-51c75654db73?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&q=80&w=1080", order=4),
        ]
        db.add_all(steps7)
        ...
        recipe8 = Recipe(
            ...
            servings=4,
            calories_per_serving=220,
            proteins_per_serving=3.0,
            fats_per_serving=20.0,
            carbohydrates_per_serving=8.0,
            image="https://images.unsplash.com/photo-1589164668747-cc0b0a5e4c0a?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&q=80&w=1080"
        )
        db.add(recipe8)
        db.flush()
        
        ingredients8 = [
            Ingredient(recipe_id=recipe8.id, name="Авокадо", amount="3", unit="шт", order=0),
            Ingredient(recipe_id=recipe8.id, name="Лимон", amount="1", unit="шт", order=1),
            Ingredient(recipe_id=recipe8.id, name="Лук репчатый", amount="0.5", unit="шт", order=2),
            Ingredient(recipe_id=recipe8.id, name="Помидор", amount="1", unit="шт", order=3),
            Ingredient(recipe_id=recipe8.id, name="Кинза", amount="3", unit="веточки", order=4),
            Ingredient(recipe_id=recipe8.id, name="Чеснок", amount="1", unit="зубчик", order=5),
            Ingredient(recipe_id=recipe8.id, name="Соль", amount="по", unit="вкусу", order=6),
            Ingredient(recipe_id=recipe8.id, name="Чипсы или хлеб", amount="для", unit="подачи", order=7),
        ]
        db.add_all(ingredients8)
        
        steps8 = [
            Step(recipe_id=recipe8.id, number=1, instruction="Разрежьте авокадо пополам, удалите косточку и выньте мякоть ложкой.", image="https://images.unsplash.com/photo-1518977822534-7049a61ee0c2?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&q=80&w=1080", order=0),
            Step(recipe_id=recipe8.id, number=2, instruction="Разомните авокадо вилкой до желаемой консистенции (можно оставить кусочки).", image="https://images.unsplash.com/photo-1551185618-07fd482ff86e?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&q=80&w=1080", order=1),
            Step(recipe_id=recipe8.id, number=3, instruction="Сразу добавьте сок лимона, чтобы авокадо не потемнело.", image="https://images.unsplash.com/photo-1609501676725-7186f70a7d28?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&q=80&w=1080", order=2),
            Step(recipe_id=recipe8.id, number=4, instruction="Мелко нарежьте лук, помидор, кинзу и чеснок. Добавьте к авокадо.", image="https://images.unsplash.com/photo-1518977822534-7049a61ee0c2?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&q=80&w=1080", order=3),
            Step(recipe_id=recipe8.id, number=5, instruction="Посолите по вкусу, аккуратно перемешайте. Подавайте с чипсами или хлебом.", image="https://images.unsplash.com/photo-1589164668747-cc0b0a5e4c0a?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&q=80&w=1080", order=4),
        ]
        db.add_all(steps8)
        
        # Рецепт 9: Сырные шарики с орехами
        recipe9 = Recipe(
            title="Сырные шарики с орехами",
            category="Закуски",
            cook_time=20,
            servings=6,
            ...
            fats_per_serving=22.0,
            carbohydrates_per_serving=6.0,
            image="https://images.unsplash.com/photo-1618164436266-4465e5fc9e96?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&q=80&w=1080"
        )
        db.add(recipe9)
        db.flush()
        
        ingredients9 = [
            Ingredient(recipe_id=recipe9.id, name="Сливочный сыр", amount="200", unit="г", order=0),
            Ingredient(recipe_id=recipe9.id, name="Твердый сыр", amount="100", unit="г", order=1),
            Ingredient(recipe_id=recipe9.id, name="Грецкие орехи", amount="50", unit="г", order=2),
            Ingredient(recipe_id=recipe9.id, name="Зеленый лук", amount="3", unit="стебля", order=3),
            Ingredient(recipe_id=recipe9.id, name="Чеснок", amount="1", unit="зубчик", order=4),
            Ingredient(recipe_id=recipe9.id, name="Паприка", amount="0.5", unit="ч.л.", order=5),
            Ingredient(recipe_id=recipe9.id, name="Соль и перец", amount="по", unit="вкусу", order=6),
        ]
        db.add_all(ingredients9)
        
        steps9 = [
            Step(recipe_id=recipe9.id, number=1, instruction="Размягчите сливочный сыр при комнатной температуре.", image="https://images.unsplash.com/photo-1551185618-07fd482ff86e?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&q=80&w=1080", order=0),
            Step(recipe_id=recipe9.id, number=2, instruction="Натрите твердый сыр на мелкой терке.", image="https://images.unsplash.com/photo-1518977822534-7049a61ee0c2?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&q=80&w=1080", order=1),
            Step(recipe_id=recipe9.id, number=3, instruction="Мелко нарежьте зеленый лук и измельчите чеснок. Обжарьте грецкие орехи и мелко порубите.", image="https://images.unsplash.com/photo-1518977822534-7049a61ee0c2?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&q=80&w=1080", order=2),
            Step(recipe_id=recipe9.id, number=4, instruction="Смешайте сливочный сыр, твердый сыр, лук, чеснок, орехи, паприку, соль и перец.", image="https://images.unsplash.com/photo-1551185618-07fd482ff86e?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&q=80&w=1080", order=3),
            Step(recipe_id=recipe9.id, number=5, instruction="Сформируйте из смеси шарики размером с грецкий орех. Охладите в холодильнике 30 минут перед подачей.", image="https://images.unsplash.com/photo-1618164436266-4465e5fc9e96?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&q=80&w=1080", order=4),
        ]
        db.add_all(steps9)
        
        # Рецепт 10: Оливки с сыром и травами
        recipe10 = Recipe(
            title="Оливки с сыром и травами",
            category="Закуски",
            cook_time=5,
            servings=4,
            ...
            fats_per_serving=12.0,
            carbohydrates_per_serving=3.0,
            image="https://images.unsplash.com/photo-1609501676725-7186f70a7d28?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&q=80&w=1080"
        )
        db.add(recipe10)
        db.flush()
        
        ingredients10 = [
            Ingredient(recipe_id=recipe10.id, name="Оливки", amount="200", unit="г", order=0),
            Ingredient(recipe_id=recipe10.id, name="Сыр фета", amount="100", unit="г", order=1),
            Ingredient(recipe_id=recipe10.id, name="Оливковое масло", amount="3", unit="ст.л.", order=2),
            Ingredient(recipe_id=recipe10.id, name="Свежий розмарин", amount="2", unit="веточки", order=3),
            Ingredient(recipe_id=recipe10.id, name="Чеснок", amount="2", unit="зубчика", order=4),
            Ingredient(recipe_id=recipe10.id, name="Лимонная цедра", amount="1", unit="ч.л.", order=5),
            Ingredient(recipe_id=recipe10.id, name="Перец черный", amount="по", unit="вкусу", order=6),
        ]
        db.add_all(ingredients10)
        
        steps10 = [
            Step(recipe_id=recipe10.id, number=1, instruction="Слейте жидкость с оливок и обсушите их.", image="https://images.unsplash.com/photo-1518977822534-7049a61ee0c2?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&q=80&w=1080", order=0),
            Step(recipe_id=recipe10.id, number=2, instruction="Нарежьте сыр фета кубиками размером с оливку.", image="https://images.unsplash.com/photo-1518977822534-7049a61ee0c2?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&q=80&w=1080", order=1),
            Step(recipe_id=recipe10.id, number=3, instruction="Мелко нарежьте розмарин и чеснок. Смешайте с оливковым маслом и лимонной цедрой.", image="https://images.unsplash.com/photo-1551185618-07fd482ff86e?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&q=80&w=1080", order=2),
            Step(recipe_id=recipe10.id, number=4, instruction="Смешайте оливки с сыром и заправкой из трав. Добавьте черный перец.", image="https://images.unsplash.com/photo-1609501676725-7186f70a7d28?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&q=80&w=1080", order=3),
            Step(recipe_id=recipe10.id, number=5, instruction="Оставьте мариноваться на 30 минут при комнатной температуре перед подачей.", image="https://images.unsplash.com/photo-1609501676725-7186f70a7d28?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&q=80&w=1080", order=4),
        ]
        db.add_all(steps10)
